...
20.3.2023 make output path optional
...
def main(args):
    try:
        if not args.inputpath or not args.band:
            raise Exception('Missing input dir argument or band number (e.g. B8A). Try --help .')
        if args.verbose:
        ...
        if args.outdir:
            ...
            out_dir_path = pathlib.Path(datadir.replace('results_','histo_'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--verbose',
                        help='Verbose information. Optional.',
                        action='store_true')
