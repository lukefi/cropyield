...
python 20-make-maps-yield.py -i /scratch/project_2009889/cropyield2024/forecasting/cloudy/predictions/2024-07-15/ \
-r gridded
...
def plotMaps(longTermNationalYield, nationalPred, data, crop, imagepath, dfeuropaFIN, pvm, pvm00, region):
    ...
    if region == 'gridded': # ei painotettu
        valueColumn = 'Prediction'
    else: # pinta-alalla painotettu ka.
        valueColumn = 'weighted_yield'
    ...
    if crop == '1310': 
        viljanimi = 'Feed barley'        
    elif crop == '1320':
        viljanimi = 'Malting barley'
    elif crop == '1400':
        viljanimi = 'Oats'
    elif crop == '1110':
        viljanimi = 'Winter wheat'
    elif crop == '1120':
        viljanimi = 'Spring wheat'
    elif crop == '1230':
        viljanimi = 'Rye'
    else:
        viljanimi = 'None'
    ...
    #print(data.head())
    data.plot(ax = ax_map, column = valueColumn, marker = 's', markersize = 25, # marker = 'o' for bullet
                       vmin = value_min, vmax = value_max, cmap = cmap, norm = norm, edgecolor = None)

    ax_map.axis('off')
    ax_map.annotate('Total mean yield:\n\n' + str(round(nationalPred)) + ' kg/ha (' + deviationNationaltxt + '%)',
    ...
    ax_map.annotate('Grey areas: data intentionally obscured due to too little or no data.',
    ...
    ax_map.set_title(viljanimi + ', mean yield (kg/ha), date: ' +  pvm, fontsize = 14, weight='bold')
    # draw colorbar
    ...
    if region == 'gridded':
        fp = os.path.join(imagepath, 'Forecast-' + crop + '-' + pvm00 + '-10kmgrid-yield.png')
    else: # maakunta
        fp = os.path.join(imagepath, 'Forecast-' + crop + '-' + pvm00 + '-yield.png')
...
def main(args):
    try:
        ...
        # pvm = pvm000.strftime("%d.%m.%Y") # Finnish date
        ...
        for crop in nationalPreds['Crop'].to_list():
            ...
            if longTermNationalYield:
                ...
                if args.region == 'maakunta':   
                    ...
                    gdfdata = pd.concat([regionborders, dfPreds2], axis = 1)
                    ...
                    # make geopandas:
                    gdfdata = gpd.GeoDataFrame(dfall3[['10kmCELLCODE', 'EOFORIGIN10km', 'NOFORIGIN10km', 'Prediction']], 
                ...
                if args.region == 'gridded':
                    boto3file = 'Forecast-' + str(crop) + '-10kmgrid-yield.png'
                    ...
                    boto3file = 'Forecast-' + str(crop) + '-' + pvm00 + '-10kmgrid-yield.png'
                    s3_resource.Object(my_bucketname, boto3file).upload_file(fp, ExtraArgs={'ACL':'public-read'})               
                else: # maakunta
                    boto3file = 'Forecast-' + str(crop) + '-yield.png'
                    print(f'Saving images into Allas {boto3file}')
                    s3_resource.Object(my_bucketname, boto3file).upload_file(fp, ExtraArgs={'ACL':'public-read'})
                    # Saving also with date:
                    boto3file = 'Forecast-' + str(crop) + '-' + pvm00 + '-yield.png'
                    s3_resource.Object(my_bucketname, boto3file).upload_file(fp, ExtraArgs={'ACL':'public-read'})
