...
python 20-make-maps-Finnish-yield.py -i /scratch/project_2009889/cropyield2024/forecasting/cloudy/predictions/2024-07-15/ \
-r gridded
...
def plotMaps(longTermNationalYield, nationalPred, data, crop, imagepath, dfeuropaFIN, pvm, pvm00, region):
    ...
    ax_map.set_title(viljanimi + ', hehtaarisatoennuste (kg/ha), tilanne ' +  pvm, fontsize = 14, weight='bold')
    # draw colorbar
    
    # draw colorbar
    cbar = fig.colorbar(mappable, cax = cb_ax, orientation='vertical', label =  'Keskisato (kg/ha)')
    ...
    if region == 'gridded':
        fp = os.path.join(imagepath, 'Satoennuste-' + crop + '-' + pvm00 + '-10kmruutu-yield.png')
    else: # maakunta
        fp = os.path.join(imagepath, 'Satoennuste-' + crop + '-' + pvm00 + '-yield.png')
        ...
        for crop in nationalPreds['Crop'].to_list():
            ...
            if longTermNationalYield:
                ...
                if args.region == 'gridded':
                    boto3file = 'Satoennuste-' + str(crop) + '-10kmruutu-yield.png'
                    ...
                    boto3file = 'Satoennuste-' + str(crop) + '-' + pvm00 + '-10kmruutu-yield.png'
                    s3_resource.Object(my_bucketname, boto3file).upload_file(fp, ExtraArgs={'ACL':'public-read'})               
                else: # maakunta
                    boto3file = 'Satoennuste-' + str(crop) + '-yield.png'
                    print(f'Saving images into Allas {boto3file}')
                    s3_resource.Object(my_bucketname, boto3file).upload_file(fp, ExtraArgs={'ACL':'public-read'})
                    # Saving also with date:
                    boto3file = 'Satoennuste-' + str(crop) + '-' + pvm00 + '-yield.png'
                    s3_resource.Object(my_bucketname, boto3file).upload_file(fp, ExtraArgs={'ACL':'public-read'})
