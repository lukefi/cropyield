...
python 20-make-maps-deviation.py -i /scratch/project_2009889/cropyield2024/forecasting/cloudy/predictions/2024-07-15/ \
-r gridded
...
def plotMaps(longTermNationalYield, nationalPred, data, crop, imagepath, dfeuropaFIN, pvm, pvm00, region):
    ...
    ax_map.set_title(viljanimi + ', deviation of mean yield (%), date: ' +  pvm, fontsize = 14, weight='bold')
    # draw colorbar
    cbar = fig.colorbar(mappable, cax = cb_ax, orientation='vertical', label =  'Mean yield compared to 8-year regional mean yield (%)')
    ...
    if region == 'gridded':
        fp = os.path.join(imagepath, 'Forecast-yield-' + crop + '-' + pvm00 + '-10kmgrid.png')
    else:
        fp = os.path.join(imagepath, 'Forecast-yield-' + crop + '-' + pvm00 + '.png')
        ...
        for crop in nationalPreds['Crop'].to_list():
            ...
            if longTermNationalYield:
                ...
                if args.region == 'gridded':
                    boto3file = 'Forecast-yield-' + str(crop) + '-10kmgrid.png'
                    ...
                    boto3file = 'Forecast-yield-' + str(crop) + '-' + pvm00 + '-10kmgrid.png'
                    s3_resource.Object(my_bucketname, boto3file).upload_file(fp, ExtraArgs={'ACL':'public-read'})               
                else:
                    boto3file = 'Forecast-yield-' + str(crop) + '.png'
                    print(f'Saving images into Allas {boto3file}')
                    s3_resource.Object(my_bucketname, boto3file).upload_file(fp, ExtraArgs={'ACL':'public-read'})
                    # Saving also with date:
                    boto3file = 'Forecast-yield-' + str(crop) + '-' + pvm00 + '.png'
                    s3_resource.Object(my_bucketname, boto3file).upload_file(fp, ExtraArgs={'ACL':'public-read'})
