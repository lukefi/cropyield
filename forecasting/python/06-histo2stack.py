...
def mergeAllGetNumpyArrays(data_folder2, data_folder3, bins, outputfile):
    ...
    for filename in os.listdir(data_folder2):
        ...
        df = df.drop(0, axis = 1)
        df = df[['parcelID', 'band','doy', *df.columns[df.columns.str.startswith("bin")]]]
        df_array.append(df)
...
def testing(all_files_df, out_dir_path, outputfile):
    ...
    print("How many farms are observed from one or several S2 granules?:", len(all_files_df[['parcelID']].drop_duplicates()))
    
    # kuinka monta tilaa mukana oikeasti?
    parcelIDs = all_files_df['parcelID'].str.rsplit('_',1).str[0]
    print("How many farms we really have?: ", len(parcelIDs.drop_duplicates()))
        
    # Kuinka monta havaintoa per tila koko kesältä, mediaani?
    print("How many observations per farm in one season (median)?: ", float(all_files_df[['parcelID', 'doy']].drop_duplicates().groupby(['parcelID']).count().median()))
