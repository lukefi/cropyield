...
26.7.2023 parcelID tallennettava farmID
...
def reshapeAndSave(full_array_stack, out_dir_path, outputfile, rank):    
    ...
    print(f"There are {len(final.farmID.str.split('_', expand = True)[1].unique())} unique farms")
    print(f"There are {len(final.farmID.unique())} farms in the data, note overlapping tiles.")
    ...
    if final[['farmID', 'band', 'doy']].duplicated(keep = False).any():
        print(f"There are {final[['farmID', 'band', 'doy']].duplicated(keep = False).sum()} duplicates out of {farms} farms. We take the first obs. only.")
        final2 = final.drop_duplicates(subset=['farmID', 'band', 'doy'], keep='first')
        final = final2.copy()
        print(f"Are there duplicates anymore: {final[['farmID', 'band', 'doy']].duplicated(keep = False).any()}")
        #tmp = final[final[['farmID', 'band', 'doy']].duplicated()]
        #print(f"{tmp.farmID.nunique()}")
        #print(f"{tmp.band.nunique()}")
        #print(f"{tmp.doy.nunique()}")
        #print(f"{tmp.farmID.str.split('_', expand=True)}")
    ...
    print(f"Shape of the 3D stack: {finalfinal.shape}")
